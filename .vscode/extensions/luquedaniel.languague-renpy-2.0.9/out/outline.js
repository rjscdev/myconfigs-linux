...
function getDocumentSymbols(document) {
    ...
    for (let type in navigationdata_1.NavigationData.data.location) {
        const category = navigationdata_1.NavigationData.data.location[type];
        let parentSymbol = new vscode_1.DocumentSymbol(type, "", getDocumentSymbolKind(type, false), range, range);
        for (let key in category) {
            if (category[key] instanceof navigation_1.Navigation) {
                if (category[key].filename === documentFilename) {
                    ...
                    let classParent = new vscode_1.DocumentSymbol(key, `:${category[key].location}`, getDocumentSymbolKind(type, true), childRange, childRange);
                    if (type === 'class') {
                ...
                if (category[key][0] === documentFilename) {
                    ...
                    let classParent = new vscode_1.DocumentSymbol(key, `:${category[key][1]}`, getDocumentSymbolKind(type, true), childRange, childRange);
                    if (type === 'class') {
                        getClassDocumentSymbols(classParent, key);
                    }
                    parentSymbol.children.push(classParent);
    ...
    if (stores) {
        let parentSymbol = new vscode_1.DocumentSymbol("store", "", getDocumentSymbolKind("store", false), range, range);
        for (let key in stores) {
            ...
            if (store instanceof navigation_1.Navigation) {
                if (store.filename === documentFilename) {
                    ...
                    let classParent = new vscode_1.DocumentSymbol(key, `:${store.location}`, getDocumentSymbolKind("store", true), childRange, childRange);
                ...
                if (store[0] === documentFilename) {
                    ...
                    let classParent = new vscode_1.DocumentSymbol(key, `:${store[1]}`, getDocumentSymbolKind("store", true), childRange, childRange);
                    getStoreDocumentSymbols(classParent, key);
                    parentSymbol.children.push(classParent);
                }
            }
