    ...
    if (checkFilenames.toLowerCase() !== "disabled") {
        ...
        if (filename) {
            if (!filename.match(/^[a-zA-Z0-9]/) || filename.startsWith('00')) {
                let invalidRange = new vscode_1.Range(0, 0, doc.lineCount, 0);
...
function checkComparisonVsAssignment(diagnostics, line, lineIndex) {
    ...
    while ((matches = rsComparisonCheck.exec(line)) !== null) {
        ...
        const diagnostic = new vscode_1.Diagnostic(range, `"=" is the equality operator. Use "==" for comparison.`, vscode_1.DiagnosticSeverity.Warning);
        diagnostics.push(diagnostic);
    }
}
function checkReservedPythonNames(diagnostics, line, lineIndex) {
    ...
    while ((matches = rxReservedPythonCheck.exec(line)) !== null) {
        ...
        const diagnostic = new vscode_1.Diagnostic(range, `"${matches[2]}" is a Python reserved name, type, or function. Using it as a variable can lead to obscure problems or unpredictable behavior.`, vscode_1.DiagnosticSeverity.Error);
        diagnostics.push(diagnostic);
    }
}
...
function checkInvalidVariableNames(diagnostics, line, lineIndex, severity) {
    ...
    while ((matches = rxVariableCheck.exec(line)) !== null) {
        if (!renpy_store.includes(matches[2])) {
            const offset = matches.index + matches[0].indexOf(matches[2]);
            const range = new vscode_1.Range(lineIndex, offset, lineIndex, offset + matches[2].length);
            const diagnostic = new vscode_1.Diagnostic(range, `"${matches[2]}": Variables must begin with a letter (and may contain numbers, letters, or underscores). Variables may not begin with '_' as Ren'Py reserves such variables for its own purposes.`, severity);
            diagnostics.push(diagnostic);
        }
    }
}
