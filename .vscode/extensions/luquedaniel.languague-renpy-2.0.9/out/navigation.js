...
function getArgumentParameterInfo(location, line, position) {
    let documentation = new vscode_1.MarkdownString();
    documentation.appendMarkdown(formatDocumentationAsMarkdown(location.documentation));
    let signature = new vscode_1.SignatureInformation(`${location.keyword}${location.args}`, documentation);
    let parsed = '';
...
function getCurrentContext(document, position) {
    ...
    let i = position.line;
    while (i >= 0) {
        let line = navigationdata_1.NavigationData.filterStringLiterals(document.lineAt(i).text);
