...
function getHover(document, position) {
    ...
    const range_key = (0, navigation_1.rangeAsString)(filename, range);
    const navigation = navigationdata_1.NavigationData.gameObjects['semantic'][range_key];
    if (navigation) {
...
function getHoverMarkdownString(locations) {
    ...
    for (let location of locations) {
        ...
        let character = navigationdata_1.NavigationData.gameObjects['characters'][location.keyword];
        if (character) {
            type = 'character';
        }
        if (location.filename && location.filename.length > 0 && location.location >= 0) {
